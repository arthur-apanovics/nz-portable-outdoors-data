 ...
 * @returns {number[]}
 */
exports.nzgd2000ToWsg86 = function (xyArray) {
  ...
  const source = {
    ...
    en: [0.9983242984231332, 0.005018678446033867, 0.00002100298118440739, 1.0936603552691419e-7, 6.178058939352553e-10],
    ml0: 0
  };
  ...
  const pj_inv_mlfn = function (arg, es, en) {
    ...
    const k = 1 / (1 - es);
    let phi = arg;
    for (let i = MAX_ITER; i; --i) { /* rarely goes over 2 iterations */
  ...
  function inverse$2(p) {
    ...
    if (Math.abs(phi) < HALF_PI) {
      ...
      lat = phi - (con * ds / (1 - source.es)) * 0.5 * (1 -
        ds / 12 * (5 + 3 * t - 9 * c * t + c - 4 * cs -
          ds / 30 * (61 + 90 * t - 252 * c * t + 45 * ts + 46 * c -
            ds / 56 * (1385 + 3633 * t + 4095 * ts + 1574 * ts * t))));

      lon = adjust_lon(source.long0 + (d * (1 -
        ds / 6 * (1 + 2 * t + c -
